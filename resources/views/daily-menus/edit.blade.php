@extends('layouts.app')
@section('title', __('Editar Cardápio'))
@section('content')

<div class="max-w-5xl mx-auto">
    <div class="page-header">
        <h1 class="page-title">{{ __('Editar Cardápio do Dia') }}</h1>
        <p class="page-subtitle">{{ __('Atualizar o cardápio de') }} {{ $menu->date instanceof \Carbon\Carbon ? $menu->date->format('d/m/Y') : $menu->date }}</p>
    </div>

    <form method="POST" action="{{ route('daily-menus.update', $menu) }}" x-data="menuBuilder({{ $menu->items->pluck('dish_id') }})">
        @csrf
        @method('PUT')
                        ...
                        <div class="rounded-xl border border-border dark:border-border-dark overflow-hidden">
                            ...
                            <div x-show="openCategories.includes({{ $category->id }})" class="divide-y divide-border dark:divide-border-dark">
                                ...
                                @php
                                    ...
                                    $menuItem = $menu->items->where('dish_id', $dish->id)->first();
                                @endphp
                                <div class="px-5 py-4 flex items-start gap-4 hover:bg-slate-50/50 dark:hover:bg-slate-700/20 transition-colors duration-150">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        @endif
                                        <div class="mt-3 grid grid-cols-3 gap-3" x-show="selectedDishes.includes({{ $dishId }}) || {{ $menuItem ? 'true' : 'false' }}" x-cloak>
                                            <div>
                                                ...
                                                <input type="number" name="price_small[{{ $dishId }}]" step="0.01" placeholder="{{ __('R$ 0,00') }}"
                                                    value="{{ old('price_small.' . $dishId, $menuItem?->price_small ?? '') }}"
                                            ...
                                            <div>
                                                ...
                                                <input type="number" name="price_medium[{{ $dishId }}]" step="0.01" placeholder="{{ __('R$ 0,00') }}"
                                                    value="{{ old('price_medium.' . $dishId, $menuItem?->price_medium ?? '') }}"
                                                    class="input-field text-sm"
                                                    x-on:input="calculateTotal()" />
                                            </div>
                                            <div>
                                                ...
                                                <input type="number" name="price_large[{{ $dishId }}]" step="0.01" placeholder="{{ __('R$ 0,00') }}"
                                                    value="{{ old('price_large.' . $dishId, $menuItem?->price_large ?? '') }}"
                                                    class="input-field text-sm"
                                                    x-on:input="calculateTotal()" />
                                            </div>
                                        </div>
...
@push('scripts')
<script>
    function menuBuilder(initialDishes = []) {
        return {
            selectedDishes: [...initialDishes],
            openCategories: [],
