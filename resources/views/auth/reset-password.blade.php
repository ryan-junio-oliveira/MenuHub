...
<x-guest-layout>
    <div class="text-center mb-8">
        <div class="w-14 h-14 rounded-2xl bg-primary-500/15 flex items-center justify-center mx-auto mb-4">
            <i class="fa-solid fa-key text-2xl text-primary-400"></i>
        </div>
        <h1 class="text-2xl font-bold text-white">{{ __('Redefinir Senha') }}</h1>
        <p class="text-sm text-slate-400 mt-1">{{ __('Escolha uma nova senha para sua conta.') }}</p>
    </div>
    ...
    <form method="POST" action="{{ route('password.store') }}">
        ...
        <div>
            ...
            <input id="email" class="input-field" type="email" name="email" :value="old('email', $request->email)" required autofocus autocomplete="username" placeholder="{{ __('seu@email.com') }}" />
