...
<div class="max-w-2xl mx-auto">
    ...
    <x-card padding="6">
        <form method="POST" action="{{ route('dishes.update', $dish) }}" class="space-y-6" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                @if ($dish->image)
                <p class="text-xs text-text-secondary mt-1">{{ __('Imagem atual:') }} {{ $dish->image }}</p>
                @endif
                <p class="text-xs text-text-secondary mt-1">{{ __('PNG, JPG ou WEBP. Máx 2MB. Deixe vazio para manter a atual.') }}</p>
            ...
            <div class="border-t border-border dark:border-border-dark pt-4">
                ...
                <div class="grid grid-cols-3 gap-4">
                    <div>
                        ...
                        <x-text-input id="price_small" name="price_small" type="number" step="0.01" class="mt-1 block w-full input-field" placeholder="{{ __('0,00') }}" :value="old('price_small', $dish->price_small)" />
                        <x-input-error :messages="$errors->get('price_small')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="price_medium" value="{{ __('Médio (R$)') }}" />
                        <x-text-input id="price_medium" name="price_medium" type="number" step="0.01" class="mt-1 block w-full input-field" placeholder="{{ __('0,00') }}" :value="old('price_medium', $dish->price_medium)" />
                        <x-input-error :messages="$errors->get('price_medium')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="price_large" value="{{ __('Grande (R$)') }}" />
                        <x-text-input id="price_large" name="price_large" type="number" step="0.01" class="mt-1 block w-full input-field" placeholder="{{ __('0,00') }}" :value="old('price_large', $dish->price_large)" />
            ...
            <div>
                ...
                <x-text-input id="max_selections" name="max_selections" type="number" placeholder="Limite de escolhas" class="mt-1 block w-full input-field" :value="old('max_selections', $dish->max_selections ?? 1)" />
